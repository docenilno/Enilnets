...
and crossover aligned by historical "innovation" markings, with speciation
protecting structural innovations. This does not reuse `NeuralNet`: NEAT
genomes are variable-topology graphs, so a `Genome` is its own minimal
feedforward representation with its own `forward()`.

GPU note: this module deliberately does NOT route through the backend `np`
proxy -- it always computes on host NumPy regardless of the process-wide
GPU switch, following `backend.default_dtype()` only for float32/float64.
Per-genome topologies differ across the population, so there is nothing to
batch, and dispatching each tiny per-node op as its own GPU kernel is far
slower (measured: an XOR run went from ~5s to ~85s under use_gpu(True))."""
import statistics
...
class NodeGene:
    ...
    def __init__(self, node_id: int, node_type: str, activation: str = "sigmoid") -> None:
        self.id = node_id
...
class InnovationTracker:
    ...
    def reset_node_id_counter(self, start: int) -> None:
    ...
    def get_node_for_split(self, connection_innovation: int) -> int:
...
class Genome:
    ...
    @classmethod
    def minimal(cls, n_inputs: int, n_outputs: int, innovation_tracker: "InnovationTracker",
                activation: str = "sigmoid", output_activation: Optional[str] = None,
                weight_range: float = 1.0) -> "Genome":
    ...
    def mutate_weights(self, rate: Optional[float] = None, perturb_rate: Optional[float] = None,
                       perturb_power: Optional[float] = None, replace_range: float = 1.0) -> None:
    ...
    def mutate_add_connection(self, innovation_tracker: "InnovationTracker", max_tries: int = 20,
                               weight_range: float = 1.0) -> bool:
    ...
    def mutate_add_node(self, innovation_tracker: "InnovationTracker") -> bool:
    ...
    def distance(self, other: "Genome", c1: Optional[float] = None, c2: Optional[float] = None,
                 c3: Optional[float] = None) -> float:
...
def crossover(fitter: Genome, other: Genome, disjoint_disable_rate: float = 0.75) -> Genome:
...
class Species:
    def __init__(self, representative: Genome, species_id: int) -> None:
        self.id = species_id
    ...
    def update_stagnation(self, best_fitness: float) -> None:
...
class NEATPopulation:
    ...
    See the README's NEAT section for a worked example."""

    def __init__(self, n_inputs: int, n_outputs: int, population_size: int = 150,
                 activation: str = "sigmoid", output_activation: Optional[str] = None,
                 compatibility_threshold: Optional[float] = None, c1: Optional[float] = None,
                 c2: Optional[float] = None, c3: Optional[float] = None,
                 weight_mutate_rate: Optional[float] = None, weight_perturb_rate: Optional[float] = None,
                 weight_perturb_power: Optional[float] = None, add_connection_rate: Optional[float] = None,
                 add_node_rate: Optional[float] = None, crossover_rate: Optional[float] = None,
                 survival_threshold: Optional[float] = None, stagnation_limit: Optional[int] = None,
                 elitism: int = 1, seed: Optional[int] = None) -> None:
        if seed is not None:
            np.random.seed(seed)
    ...
    def evolve(self, fitness_fn: Any, generations: int = 100, verbose: bool = True) -> List[float]:
