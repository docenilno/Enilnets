...
from ..text.text_utils import one_hot_encode as one_hot
...
def train_test_split(X: Any, Y: Any, test_size: Union[float, int] = 0.2, shuffle: bool = True,
...
def k_fold_split(X: Any, Y: Any, k: int = 5, shuffle: bool = True,
                  seed: Optional[int] = None) -> Generator[Tuple[Any, Any, Any, Any], None, None]:
...
def iterate_minibatches(X: Any, Y: Any, batch_size: int, shuffle: bool = True,
                         seed: Optional[int] = None) -> Generator[Tuple[Any, Any], None, None]:
...
def count_parameters(model: Any) -> Tuple[int, Dict[int, Dict[str, Any]]]:
...
class ModelCheckpoint:
    ...
    mode: "min" (e.g. loss) or "max" (e.g. accuracy).
    """
    def __init__(self, monitor: str = "val_loss", mode: str = "min", min_delta: float = 0.0) -> None:
    ...
    def on_epoch_end(self, epoch: int, logs: Dict[str, float], model: Optional[Any] = None) -> None:
