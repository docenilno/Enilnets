"""NeuralNet.Save/Load: full trainable-state serialization to .pkl or
.json."""
import json
import pickle
import os
...
_LAYER_ARRAY_KEYS = [
    ...
    "Wx", "Wh", "b", "bx", "bh",
    "W1", "b1", "W2", "b2", "Wr", "br",   # MoE experts + router; CBAM's shared MLP
    "omega",   # Performer random-feature matrix: fixed, but it DEFINES the
               # layer's function, so it has to round-trip like a weight.
]

def _numpy_encoder(obj: Any) -> List[Any]:
    if backend.is_array(obj):
        return backend.to_numpy(obj).tolist()
    raise TypeError(f"Object of type {type(obj)} is not JSON serializable")

def _host_layers(layers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # "_state_*" keys are stateful-RNN carried hidden state -- transient
    # runtime data tied to whatever stream the model was mid-way through,
    # not part of the trainable model. Excluded from save files entirely
    # (a freshly loaded model starts its streams from zero, like Keras).
    return [
        {k: (backend.to_numpy(v) if backend.is_array(v) else v)
         for k, v in layer.items() if not k.startswith("_state_")}
        for layer in layers
    ]

def _host_state_list(states: List[Optional[Dict[str, Any]]]) -> List[Optional[Dict[str, Any]]]:
...
def _restore_grad_accum(raw_accum: Optional[List[Optional[Dict[str, Any]]]], dtype: Any) -> List[Optional[Dict[str, Any]]]:
    if not raw_accum:
        return []
    restored = []
...
def Save(self: Any, file: str, save_opt_state: bool = True, extra_state: Optional[Dict[str, Any]] = None) -> None:
    """Save the model's full trainable state to `file` (.pkl or .json): layer
    weights, every per-model optimizer/training hyperparameter, optimizer
    state, in-progress gradient-accumulation buffers, the shape-inference and
    residual bookkeeping needed to keep building the model after loading, and
    the training/eval flag.

    `save_opt_state` controls the momentum/velocity buffers; `extra_state` is
    an optional dict saved alongside (EMA weights, persistent CD buffer, ...)
    and handed back by Load()."""
    payload = {
        "version": 6,
...
def Load(self: Any, file: str, load_opt_state: bool = True) -> Optional[Dict[str, Any]]:
    """Restore everything Save() wrote from `file`. Returns the `extra_state`
    dict that was saved with it, or None. `load_opt_state` controls whether
