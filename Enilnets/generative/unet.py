...
class UNetDenoiser:
    def __init__(self, in_ch, base_ch=64, time_emb_dim=128, ch_mult=(1, 2, 4),
        ...
        for i, mult in enumerate(ch_mult):
            ...
            net.add_conv2d(prev_ch, out_ch, k=1, activation="swish", init_method=init_method)
            net.add_conv2d(out_ch, out_ch, k=1, activation="swish", init_method=init_method)
        ...
        self.bottleneck.add_conv2d(bot_ch, bot_ch, k=1, activation="swish", init_method=init_method)
        self.bottleneck.add_conv2d(bot_ch, bot_ch, k=1, activation="swish", init_method=init_method)
        ...
        for i in reversed(range(len(ch_mult))):
            ...
            net = NeuralNet(**net_kwargs)
            net.add_conv2d(in_ch_dec + out_ch, out_ch, k=1, activation="swish", init_method=init_method)
            net.add_conv2d(out_ch, out_ch, k=1, activation="swish", init_method=init_method)
            self.decoders.append(net)

        # Output conv
        self.out_net = NeuralNet(**net_kwargs)
        self.out_net.add_conv2d(base_ch * ch_mult[0], in_ch, k=1, activation="linear", init_method=init_method)
